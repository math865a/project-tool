...
export class SyncBookingsHandler implements ICommandHandler<SyncBookingsCommand, void> {
    constructor(
    ...
    async execute(command: SyncBookingsCommand): Promise<void> {
        ...
        const workpackageId: string | undefined = queryResult.records[0]?.get('workpackageId');
        const agentId: string | undefined = queryResult.records[0]?.get('agentId');
