...
export class WorkpackageProfileQueryHandler implements IQueryHandler<WorkpackageProfileQuery> {
    constructor(private client: Neo4jClient) {}

    async execute(query: WorkpackageProfileQuery): Promise<any> {
        return await Promise.all([this.getProfile(query.workpackageId), this.getOptions()]).then(
            (res) => ({
                ...res[0],
                options: res[1],
            }),
