...
import { Activity, Gantt } from "@libs/features";
import { DateTime as dt, Interval as int } from "luxon";
import { getDateTime, normalize } from "@libs/util";
...
export class ActivityPeriod extends Model(
    ...
    onAttachedToRootStore() {
        const encloseChildren = reaction(
    ...
    @computed
    get periodState() {
        if (!this.Timeline.xScale || !this.Activity)
            return {
                start: dt.now(),
    ...
    get displayInterval() {
        return (
            this.periodState.start.toFormat("d/M/yy") +
            "-" +
            this.periodState.end.toFormat("d/M/yy")
        );
    }
